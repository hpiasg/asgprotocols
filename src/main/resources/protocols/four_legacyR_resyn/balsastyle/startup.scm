...
;;;
;;;	`startup.scm'
;;; four_legacyR_resyn style startup
;;;
...
(for-each (lambda (component)
   	(if (list? component) ; (name from-name from-style)
		(brz-add-primitive-part-implementation (car component)
			`(style "four_legacyR_resyn" (include style ,(caddr component) ,(cadr component)))
		)
		(brz-add-primitive-part-implementation component
			`(style "four_legacyR_resyn" (include style "four_legacyR_resyn" ,component))
		)
    )
) '(
